...
export default function Dashboard() {
  ...
  const [newContact, setNewContact] = useState({ name: "", email: "", phone: "", alias: "" })
  ...
  const handleAddContact = async () => {
    if (!newContact.name) {
      alert("Please provide at least one contact name.")
      return
    }
    ...
    // });
    setNewContact({ name: "", email: "", phone: "", alias: "" })
    ...
    >
      ...
      {isAddContactModalOpen && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-slate-900/80 border border-slate-700/50 backdrop-blur-sm rounded-lg p-8 w-full max-w-md">
            <h2 className="text-xl font-bold text-cyan-400 mb-4">Add New Contact</h2>
            <div className="space-y-4">
              <Input type="text" placeholder="Name" name="name" value={newContact.name} onChange={handleContactInputChange} className="bg-slate-800/50 border-slate-700/50" />
              <Input type="email" placeholder="Email" name="email" value={newContact.email} onChange={handleContactInputChange} className="bg-slate-800/50 border-slate-700/50" />
              <Input type="text" placeholder="Phone Number" name="phone" value={newContact.phone} onChange={handleContactInputChange} className="bg-slate-800/50 border-slate-700/50" />
              <Input type="text" placeholder="Alias" name="alias" value={newContact.alias} onChange={handleContactInputChange} className="bg-slate-800/50 border-slate-700/50" />
            </div>
            <div className="flex justify-end space-x-4 mt-6">
              <Button variant="ghost" onClick={() => setIsAddContactModalOpen(false)}>Cancel</Button>
              <Button className="bg-cyan-600 hover:bg-cyan-700" onClick={handleAddContact}>Add Contact</Button>
            </div>
          </div>
        </div>
      )}
            ...
            <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm h-full">
              <CardContent className="p-4">
                <nav className="space-y-2">
                  ...
                  <NavItem icon={Terminal} label="Console" active={activeTab === "Console"} onClick={() => setActiveTab("Console")} />
                  <NavItem icon={MessageSquare} label="Communications" active={activeTab === "Communications"} onClick={() => setActiveTab("Communications")} />
          ...
          <div className="col-span-12 md:col-span-9 lg:col-span-7">
            ...
            {activeTab === 'Settings' && settings && (
              <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-cyan-400">Groq API Keys</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {settings.api_keys.groq.map((key: any) => (
                        <div key={key.slot} className={`p-4 rounded-lg ${key.has_key ? 'bg-slate-800' : 'bg-slate-900 opacity-50'}`}>
                          <div className="flex justify-between items-center">
                            <span className="font-mono">{key.label}</span>
                            <Badge variant={key.has_key ? (key.is_active ? "default" : "outline") : "destructive"}>
                              {key.has_key ? (key.is_active ? "Active" : "Available") : "Not Set"}
                            </Badge>
                          </div>
                        </div>
                      ))}
                    </div>
                    <h3 className="text-lg font-semibold text-cyan-400 mt-6">Google API Keys</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {settings.api_keys.google.map((key: any) => (
                        <div key={key.slot} className={`p-4 rounded-lg ${key.has_key ? 'bg-slate-800' : 'bg-slate-900 opacity-50'}`}>
                          <div className="flex justify-between items-center">
                            <span className="font-mono">{key.label}</span>
                            <Badge variant={key.has_key ? (key.is_active ? "default" : "outline") : "destructive"}>
