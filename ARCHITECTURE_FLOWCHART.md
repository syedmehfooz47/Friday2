...
- API key slots (15 Google + 10 Groq)
- Active API keys
...
GET /api/chatlogs?date=2026-01-11
```
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/generated-files` | List all generated/converted files with metadata |
...
| `set_brightness` | `{brightness: number}` | Set screen brightness |
...
| `generate_pdf` | `{title: string, content: string}` | Generate PDF |
| `generate_word` | `{title: string, content: string}` | Generate Word doc |
| `generate_ppt` | `{title: string, slides: array}` | Generate PowerPoint |
| `generate_excel` | `{filename: string, data: array}` | Generate Excel |
| `generate_image` | `{prompt: string}` | Generate AI image |
| `convert_file` | `{input_path: string, output_format: string}` | Convert file |
| `send_email` | `{to: string, subject: string, body: string}` | Send email |
| `send_telegram` | `{chat_id: string, message: string}` | Send Telegram |
| `add_contact` | `{name: string, phone: string, email: string}` | Add contact |
| `find_contact` | `{name: string}` | Find contact |
| `recall_memory` | `{query: string}` | Query long-term memory |
| `toggle_mic` | `{is_muted: boolean}` | Toggle mic state |
| `get_state` | `{}` | Get voice assistant state |
| `stop_jarvis` | `{}` | Stop assistant speaking |
...
| `chatlogs_response` | `{chatlogs: array}` | Chat history |
| `new_message` | `{role: string, content: string}` | New chat message |
| `weather_update` | `{...weatherData}` | Weather information |
| `brightness_changed` | `{brightness: number}` | Brightness updated |
| `theme_changed` | `{theme: string}` | Theme changed |
| `screenshot_taken` | `{file_path: string}` | Screenshot saved |
| `app_opened` | `{message: string}` | App opened |
| `pdf_generated` | `{file_path: string}` | PDF created |
| `image_generated` | `{image_paths: array}` | Images created |
| `file_converted` | `{file_path: string}` | File converted |
| `email_sent` | `{status: string}` | Email sent |
| `contact_added` | `{status: string}` | Contact added |
| `mic_toggled` | `{is_muted: boolean}` | Mic state changed |
| `mic_toggle_failed` | `{error: string}` | Mic toggle failed |
| `jarvis_stopped` | `{success: boolean}` | Assistant stopped speaking |
| `system_metrics` | `{cpuUsage, memoryUsage, networkStatus}` | System stats (broadcast every 2s) |
...
curl -X POST http://localhost:8000/api/contacts \
  -H "Content-Type: application/json" \
  -d '{"names": ["Alice"], "email": "alice@example.com"}'

# Get chat logs
curl "http://localhost:8000/api/chatlogs?limit=50"
...
ws.onopen = () => {
  ...
  ws.send(JSON.stringify({
    type: 'send_message',
    payload: { message: 'What is the weather in Paris?' }
  }));
};
...
ws.onmessage = (event) => {
  ...
  switch(data.type) {
    case 'new_message':
      ...
      break;
    case 'system_metrics':
      console.log('System:', data.payload);
      break;
    case 'mic_toggled':
      console.log('Mic state:', data.payload.is_muted);
      break;
  }
};
...
print(f"Mic state: {response.json()}")
